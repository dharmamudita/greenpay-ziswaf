...
router.put('/me', authenticateToken, async (req, res) => {
  try {
    const { display_name, phone, address, photo_url } = req.body;
    const result = await pool.query(
      `UPDATE users SET display_name = COALESCE($1, display_name), phone = COALESCE($2, phone), 
       address = COALESCE($3, address), photo_url = COALESCE($4, photo_url), updated_at = CURRENT_TIMESTAMP 
       WHERE id = $5 RETURNING id, email, display_name, role, photo_url, phone, address, green_points`,
      [display_name, phone, address, photo_url, req.user.id]
    );
